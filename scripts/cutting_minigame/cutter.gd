...
var particles = preload("res://scenes/effects/cpu_particles_2d.tscn")
...
# Returns the blade's four corners in global space, used to check its footprint.
func get_blade_footprint_global() -> PackedVector2Array:
	var pivot_offset = CuttingConfig.BLADE_WIDTH / 2.0
	var forward = Vector2(cos(cut_direction), sin(cut_direction))
	var perpendicular = Vector2(-forward.y, forward.x)
	var center = current_position + forward * (CuttingConfig.BLADE_LENGTH / 2.0 - pivot_offset)

	var half_length = CuttingConfig.BLADE_LENGTH / 2.0
	var half_width = CuttingConfig.BLADE_WIDTH / 2.0
	var corners = PackedVector2Array([
		center + forward * half_length + perpendicular * half_width,
		center + forward * half_length - perpendicular * half_width,
		center - forward * half_length - perpendicular * half_width,
		center - forward * half_length + perpendicular * half_width,
	])
	for i in range(corners.size()):
		corners[i] = to_global(corners[i])
	return corners
